...
def calculate_largest_blur_area(image_path, img):
    ...
    hsv_image = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)     #HSV stands for Hue(represents the color itself, such as red, green, blue), Saturation(purity of the color), and Value(Brightness)
...
def evaluate_model():
    if request.method == 'POST':
        ...
        # Read the training history from the CSV file
        ...
        # #plt.show()
        # plt.savefig('static/model_loss.png')  # Save the graph as an image
        # plt.close()
...
def detect():
    if request.method == 'POST':
        ...
        if class_names[np.argmax(score)]!='good':
            ...
            # results.append(f"Area: {largest_blur_area/100} mm")
            calculate_largest_blur_area(path, image)
            images.append("http://127.0.0.1:5000/static/blur/"+image)
